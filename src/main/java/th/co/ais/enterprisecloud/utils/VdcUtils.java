...
public class VdcUtils {
	 ...
	 */
	public AdminVdc addPayAsYouGoVdc(OrganizationType org, VcloudAdmin adminClient, VcloudClient client, AdminOrganization adminOrg, VcloudProperties prop)
		...
		if(org.getCloudResources() != null && org.getCloudResources().getProviderVdc() != null && org.getCloudResources().getProviderVdc().getName() != null)
			...
			providerVdc = prop.getProviderVdc();
		...
		if (org.getCloudResources() != null 
			...
			networkPool = prop.getNetworkPool();
		...
		if(org.getVdc() != null && org.getVdc().getName() != null)
			...
			if (org.getVdc() == null) {
				th.co.ais.enterprisecloud.domain.VdcType vdc = new th.co.ais.enterprisecloud.domain.VdcType();
				org.setVdc(vdc);
			}
		...
		if(org.getVdc() != null && org.getVdc().getDescription() != null)						
			...
			createVdcParams.setDescription("VDC for " + org.getName());
		...
		for (ReferenceType providerVdcStorageProfileRef : pvdc.getProviderVdcStorageProfileRefs()) {
			...
			if (providerStorageProfile.getResource().getName().equalsIgnoreCase(prop.getProviderStorageProfile())) {
		...
		logger.info("Creating Pay As You Go Vdc : " + adminVdc.getResource().getName() + " : "
	 ...
	 * @throws VCloudException
	 */
	public Vdc findVdc(VcloudClient client, String orgName, String vdcName) throws VCloudException {
	...
	public String generateVdcName(OrganizationType org){
